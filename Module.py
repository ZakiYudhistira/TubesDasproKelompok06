...
def load_data(nama_file:str, n_param:int, n_max:int):
...
def cekNamaJin(file_user_utama,nama_jin):
    ...
    for i in range(file_user_utama.n_max):
...
def ubahBahan(file_bahan_utama,pasir,batu,air):
    ...
    for i in range(file_bahan_utama.n_max):
...
def cariSlotCandi(file_candi_utama):
    ...
    kosong = True
    for i in range(file_candi_utama.n_max):
...
def hapusJin(file_user_utama):
    ...
    if cekNamaJin(file_user,nama_jin):
        ...
        if command == "Y":
            ...
            for i in range(Id_jin,(file_user_utama.n_max-1)):
                file_user[i] = file_user[i+1]
            file_user[file_user_utama.n_max-1] = [None,None,None]
            printHapus(nama_jin)
        elif command == "N":
